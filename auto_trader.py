...
#取得市值差阀值
def getShizhiCha(str_time):
    cha = float(os.environ['cha']) if float(os.environ['cha']) else 3
    
    #闭市前几分钟无法看到五档行情，提高阀值保障安全性
    if str_time >= '14:56' and str_time <= '14:59':
        cha += 0.5
...
def autoTrader(position_info,min_liutong,cha):
    ...
    #满足差价(4%)才交易
    if cha <= getShizhiCha(min_liutong['datetime'][1:6]) :
        print ('cha %s' % cha)
        return
    ...
    try:
        ...
        #插入leancloud的非关系型数据库
        saveTradeHistoryLeanCloud(min_liutong,'B')
        saveTradeHistoryLeanCloud(position_info,'S')
